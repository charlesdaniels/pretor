...
class PSF:
    ...
    def load_from_archive(this, archive_path: pathlib.Path):
        ...
        with zipfile.ZipFile(str(archive_path), "r") as f:
            ...
            if psf_format_revision == 0:
                return this.load_from_archive_rev0(f, archive_path)
            else:
                logging.error(
    ...
    def load_from_archive_rev0(this, arc: zipfile.ZipFile, archive_path: pathlib.Path):
        """load_from_archive_rev0

        Populate this PSF from an archive with format revision 0

        :param this:
        :param arc:
        :type arc: zipfile.ZipFile
        :param archive_path: path from which the archive was loded
        ...
        try:
            ...
            util.log_exception(e)
            logging.warning(
                "archive {} has missing or invalid forensic data".format(archive_path)
            )
        ...
        try:
            ...
            raise PSFInvalid("Invalid archive {}, no pretor_data.toml".format(arc))

        pretor_data = None
        try:
            pretor_data = toml.loads(arc.read("pretor_data.toml").decode("utf-8"))
        except Exception as e:
            util.log_exception(e)
            raise PSFInvalid(
    ...
    def load_revision_from_archive_rev0(this, arc: zipfile.ZipFile, revID: str):
        ...
        Load a revision from a format revision 0 archive file.
        ...
        if "archive_name" in this.metadata:
            archive_path = this.metadata["archive_name"]
        ...
        for path in rev_data["contents"]:
            ...
            try:
                rev.put_file(path, arc.read(full_path))
            except Exception as e:
                util.log_exception(e)
                raise PSFInvalid(
