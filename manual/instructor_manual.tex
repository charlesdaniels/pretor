...
\pretoremph{
	\textbf{Note} When exiting an interactive Bash grading session, not
	only changes to \texttt{grade.toml}, but also to anything within the
	\texttt{contents/} directory is saved as a new revision.
	\texttt{pretor-psf -{}-diff} can be used to view changes between
	different revisions. This can be a valuable feedback mechanism --
	it can be useful to annotate student code with comments and provide
	them with the diff as feedback, or to track changes that needed to
	be made to get the code to compile or pass a test suite.
}

\pretoremph{
	\textbf{Note} When the \texttt{contents/} directory is saved after an
	interactive session, any exclude patterns defined in
	\texttt{pretor.toml} will be honored, to avoid adding build artifacts
	to the generated revision. You can modify the \texttt{pretor.toml}
	within the interactive session if you wish to exchange the list of
	excluded patterns. See $\S$\ref{sec:pretortoml} for more information.
}
...
\subsection{Course Definition File Schema}

\begin{verbatim}
[course]
name = "Your course name here (i.e. CS101)"
description = "Optional course description"

[arbitrary string other than 'course']
name = "Assignment name (i.e. 'Project 2')"
description = "Optional description here"
weight = 0.0  # not used, slated for deprecation
ruberic_category_1 = 10  # specify number of marks for ruberic category 1
ruberic_category_2 = 20
# ...
ruberic_category_n = 5
\end{verbatim}
...
\pretoremph{ \textbf{Note}: There is considerable overlap between
\texttt{pretor-query} and \texttt{pretor-export}. This is by design; the latter
serves a narrower, more common use case where the improved ease-of-use will
prove valuable. The former is ostensibly more flexible, but requires a
knowledge of SQL, and is a lower level abstraction, more closely reflecting the
internal representation of PSFs within Pretor.}
...
\pretoremph{\textbf{Note}: Pretor tracks any metadata edits made by storing
them in the \texttt{modifymetadata} field of the forensic data. It is always
possible to determine the original metadata of a PSF by viewing this field.}

\section{Importing Existing Grade Data With \texttt{pretor-import}}

It is sometimes convenient to maintain or create grade data separately from
Pretor. However, even in such cases, it is often useful to use Pretor as an
archival tool for storing grade data and submissions in a combined format.  To
support this use case, Pretor includes a tool for folding CSV or TSV formatted
grading information into a collection of PSFs.

\texttt{pretor-import} is an intelligent tool -- the input data need only
specify enough information about a given grade record to uniquely identify a
single PSF in the collection of those being considered. In some cases, such as
a collection of PSFs for a specific course, section, semester, and assignment,
only the group would be sufficient to associate grade records with PSFs.

The schema of the input data must be specified, either as a header row in the
CSV/TSV, or as a parameter. The schema may specify any field that would
normally be contained in a \texttt{pretor.toml} (used to associate records with
PSFs), any field that would normally be used to specify a grade modifier (such
as override, bonus, or penalty), or any category (only if the category name is
not also a valid metadata field name or a valid grade modifier field name) A
full list of relevant keys is provided in the output of \texttt{pretor-import
-{}-help}.

Note that \texttt{pretor-import} will only match records to PSFs if all columns
specified in the schema are present in the PSF, and that all such columns have
identical values to that of the corresponding PSF (i.e. fuzzy matching is not
supported).
